-- H2N V9.8 | Fixed | Ragdoll TP with stops | Enhanced AntiRagdoll | Full Save
...
-- ====== RAGDOLL TP (مدة الوقوف: L1/R1 = 0.2, L2/R2 = 0.15) ======
...
local function teleportSequence(side)
    ...
    task.wait(0.2)   -- L1/R1 وقوف 0.2 ثانية
    hrp.CFrame = CFrame.new(points[2])
    DuelTP.LastTPTime = tick()
    task.wait(0.15)  -- L2/R2 وقوف 0.15 ثانية
...
local function getHRPGrab() local c = LP.Character return c and (c:FindFirstChild("HumanoidRootPart") or c:FindFirstChild("UpperTorso")) end
...
local function buildCallbacks(prompt)
    ...
    if ok1 and type(c1)=="table" then
        for _, conn in ipairs(c1) do if type(conn.Function)=="function" then table.insert(data.holdCBs, conn.Function) end end
    end
    local ok2, c2 = pcall(getconnections, prompt.Triggered)
    if ok2 and type(c2)=="table" then
        for _, conn in ipairs(c2) do if type(conn.Function)=="function" then table.insert(data.triggerCBs, conn.Function) end end
    end
    if #data.holdCBs > 0 or #data.triggerCBs > 0 then InternalStealCache[prompt] = data end
end
...
local function firePrompt(prompt)
    local fired = false
    pcall(function()
        if fireproximityprompt then fireproximityprompt(prompt); fired = true end
    end)
    if fired then return end
    pcall(function()
        prompt:InputHoldBegin()
        task.delay(0.05, function() pcall(function() prompt:InputHoldEnd() end) end)
    end)
end
...
local function StartAutoGrab()
    ...
    grabMainConn = RunService.Heartbeat:Connect(function(dt)
        if not State.AutoGrab then
            grabMainConn:Disconnect(); grabMainConn = nil
            return
        end
        if not HRP then return end
...
-- Visual Tab (مع إعدادات الحجم والإظهار/الإخفاء)
...
-- إضافة إعدادات Visual
local sepVisual = Instance.new("TextLabel", visual)
sepVisual.Size = UDim2.new(1, -10, 0, 24)
sepVisual.Position = UDim2.new(0, 5, 0, vi * 44 + 4)
sepVisual.BackgroundTransparency = 1
sepVisual.Text = "───────── GUI SETTINGS ─────────"
sepVisual.TextColor3 = Color3.fromRGB(0, 170, 255)
sepVisual.Font = Enum.Font.GothamBold
sepVisual.TextSize = 12
sepVisual.TextTransparency = 0.3
sepVisual.ZIndex = 11
vi = vi + 1

-- إظهار/إخفاء المينيو
local menuToggleRow = Instance.new("Frame", visual)
menuToggleRow.Size = UDim2.new(1, -10, 0, 40)
menuToggleRow.Position = UDim2.new(0, 5, 0, vi * 44 + 4)
menuToggleRow.BackgroundTransparency = 1
menuToggleRow.ZIndex = 11

local menuToggleLbl = Instance.new("TextLabel", menuToggleRow)
menuToggleLbl.Size = UDim2.new(0.7, 0, 1, 0)
menuToggleLbl.Position = UDim2.new(0, 10, 0, 0)
menuToggleLbl.BackgroundTransparency = 1
menuToggleLbl.Text = "Show Menu"
menuToggleLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
menuToggleLbl.Font = Enum.Font.GothamBold
menuToggleLbl.TextSize = 13
menuToggleLbl.TextXAlignment = Enum.TextXAlignment.Left
menuToggleLbl.ZIndex = 12

local menuToggleBtn = Instance.new("TextButton", menuToggleRow)
menuToggleBtn.Size = UDim2.new(0.25, 0, 0.7, 0)
menuToggleBtn.Position = UDim2.new(0.72, 0, 0.15, 0)
menuToggleBtn.BackgroundColor3 = Color3.fromRGB(50, 0, 0)
menuToggleBtn.Text = "HIDE"
menuToggleBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
menuToggleBtn.Font = Enum.Font.GothamBold
menuToggleBtn.TextSize = 12
menuToggleBtn.ZIndex = 12
...
menuToggleBtn.MouseButton1Click:Connect(function()
    ...
    if menuVisible then
        ...
        menuToggleBtn.Text = "SHOW"
        menuToggleBtn.BackgroundColor3 = Color3.fromRGB(180, 0, 0)
        menu.Visible = false
    end
end)
vi = vi + 1
...
local menuSizeBox = Instance.new("TextBox", menuSizeRow)
menuSizeBox.Size = UDim2.new(0.25, 0, 0.7, 0)
menuSizeBox.Position = UDim2.new(0.72, 0, 0.15, 0)
menuSizeBox.BackgroundColor3 = Color3.fromRGB(10, 0, 0)
menuSizeBox.Text = tostring(menuW) .. "x" .. tostring(menuH)
menuSizeBox.TextColor3 = Color3.fromRGB(255, 255, 255)
menuSizeBox.Font = Enum.Font.GothamBold
menuSizeBox.TextSize = 12
menuSizeBox.ZIndex = 12
Instance.new("UICorner", menuSizeBox).CornerRadius = UDim.new(0, 8)

menuSizeBox.FocusLost:Connect(function()
    local w, h = menuSizeBox.Text:match("(%d+)x(%d+)")
    if w and h then
        menuW = tonumber(w)
        menuH = tonumber(h)
        menu.Size = UDim2.new(0, menuW, 0, menuH)
...
-- إظهار/إخفاء شريط السرقة
local stealBarRow = Instance.new("Frame", visual)
stealBarRow.Size = UDim2.new(1, -10, 0, 40)
stealBarRow.Position = UDim2.new(0, 5, 0, vi * 44 + 4)
stealBarRow.BackgroundTransparency = 1
stealBarRow.ZIndex = 11

local stealBarLbl = Instance.new("TextLabel", stealBarRow)
stealBarLbl.Size = UDim2.new(0.7, 0, 1, 0)
stealBarLbl.Position = UDim2.new(0, 10, 0, 0)
stealBarLbl.BackgroundTransparency = 1
stealBarLbl.Text = "Show Steal Bar"
stealBarLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
stealBarLbl.Font = Enum.Font.GothamBold
stealBarLbl.TextSize = 13
stealBarLbl.TextXAlignment = Enum.TextXAlignment.Left
stealBarLbl.ZIndex = 12

local stealBarBtn = Instance.new("TextButton", stealBarRow)
stealBarBtn.Size = UDim2.new(0.25, 0, 0.7, 0)
stealBarBtn.Position = UDim2.new(0.72, 0, 0.15, 0)
stealBarBtn.BackgroundColor3 = StealBarVisible and Color3.fromRGB(180, 0, 0) or Color3.fromRGB(50, 0, 0)
stealBarBtn.Text = StealBarVisible and "HIDE" or "SHOW"
stealBarBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
stealBarBtn.Font = Enum.Font.GothamBold
stealBarBtn.TextSize = 12
stealBarBtn.ZIndex = 12
Instance.new("UICorner", stealBarBtn).CornerRadius = UDim.new(0, 8)

stealBarBtn.MouseButton1Click:Connect(function()
...
-- الأزرار الجانبية (UNWALK تمت إزالته)
...
-- ====== زر RAGDOLL TP الجانبي (مع زر L/R) ======
do
    ...
    arrowBtn.MouseButton1Click:Connect(function()
        ...
        dirLbl.Text = (DuelTP.EnemySide == "RIGHT") and "→ R" or "← L"
        arrowBtn.Text = (DuelTP.EnemySide == "RIGHT") and "R" or "L"
        if tpSV then tpSV.Text = (DuelTP.EnemySide == "LEFT") and "← LEFT" or "RIGHT →" end
...
print("H2N V9.8 - Final | UNWALK removed from side buttons | Ragdoll TP stops: 0.2 / 0.15 | All side buttons draggable | Visual settings added")