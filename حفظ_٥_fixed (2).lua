-- H2N V10 | Ultimate Edition | Auto Duel No Delay | No Home Set | Fixed Toggles | Dual AutoDuel System
...
-- ====== AUTO DUEL العادي (5 نقاط) ======
local aplConn, aprConn = nil, nil
local aplPhase, aprPhase = 1, 1

-- ====== POST-TP AUTO DUEL (4 نقاط) - مستقل تماماً ======
-- هذا النظام منفصل كلياً عن الأوتو دولز العادي
-- له variables خاصة فيه ولا يتداخل مع aplConn/aprConn
local postTPConn   = nil   -- الكونكشن الخاص بالأوتو دولز بعد التيليبورت
local postTPPhase  = 1     -- الفاز الخاص فيه
local postTPActive = false -- حالته الخاصة
...
-- ====== إيقاف Post-TP Auto Duel ======
local function StopPostTPDuel()
    postTPActive = false
    if postTPConn then
        postTPConn:Disconnect()
        postTPConn = nil
    end
    postTPPhase = 1
    local h = getHRP2()
...
-- ====== تحديث الأوتو دولز العادي - Right ======
...
-- ====== تحديث Post-TP Auto Duel (4 نقاط) - منفصل تماماً ======
local postTPRoute = {}  -- يتحدد لما يبدأ بناءً على الجهة

local function updatePostTPDuel()
    if not postTPActive then
        if postTPConn then
            postTPConn:Disconnect()
            postTPConn = nil
        end
    ...
    local target = postTPRoute[postTPPhase]
    if not target then
        StopPostTPDuel()
        return
    end
    ...
    -- كل النقاط الـ 4 بسرعة الدولز العادية
    local spd = DUEL_RETURN_SPD
    local reached = MoveToPoint(h, targetPos, spd)

    if reached then
        postTPPhase = postTPPhase + 1
        if postTPPhase > #postTPRoute then
            StopPostTPDuel()
        end
    end
end

-- ====== تشغيل الأوتو دولز العادي ======
local function StartAutoPlayLeft()
    ...
    -- لا نوقف Post-TP Duel، كل واحد مستقل
...
-- ====== RAGDOLL TP ======
-- التيليبورت: نقطة 1 → انتظار 0.2 | نقطة 2 → انتظار 0.15
-- بعد التيليبورت يشغّل Post-TP Duel (4 نقاط) المستقل
...
local function teleportSequence(side)
    ...
    -- التيليبورت للنقطة الأولى (L1 أو R1) مع تأخير 0.17
    hrp.CFrame = CFrame.new(points[1])
    task.wait(0.17)

    -- التيليبورت للنقطة الثانية (L2 أو R2) مع تأخير 0.13
...
-- حالة لمنع تشغيل onRagdollHit أكثر من مرة بنفس الوقت
local tpInProgress = false
local duelTPPaused = false

local function StartPostTPDuel(side)
    -- إيقاف الأوتو دولز العادي إذا كان شغال
    if State.AutoPlayLeft then StopAutoPlayLeft() end
    if State.AutoPlayRight then StopAutoPlayRight() end

    -- إيقاف أي Post-TP Duel سابق
    if postTPActive then StopPostTPDuel() end

    -- تحديد المسار بناءً على الجهة (4 نقاط) - يبدأ من النقطة الثانية بعد التيليبورت مباشرة
    if side == "LEFT" then
        postTPRoute = {"L1", "R1", "R2", "L2"}
        Notify("⚡ Post-TP: L1→R1→R2→L2")
    else
        postTPRoute = {"R1", "L1", "L2", "R2"}
        Notify("⚡ Post-TP: R1→L1→L2→R2")
    end

    postTPPhase = 1
    postTPActive = true

    -- تأمين AutoRotate مع فحص الـ Hum
    local char = LP.Character
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    if hum then hum.AutoRotate = false
    elseif Hum then Hum.AutoRotate = false end

    if postTPConn then postTPConn:Disconnect() end
    postTPConn = RunService.Heartbeat:Connect(updatePostTPDuel)
end

-- الدالة الرئيسية لما ينضرب - تعمل في كل الحالات لما DuelTP مفعل
local function onRagdollHit()
    if not State.DuelTP then return end
    if tpInProgress then return end
    if not DuelTP.EnemySide then return end

    tpInProgress = true
    duelTPPaused = true  -- أوقف الكشف عن ragdoll فوراً
    ...
    local success = teleportSequence(side)

    tpInProgress = false

    if success then
        -- استنى تخرج من الـ ragdoll (أقصاه 1.5 ثانية)
        local char = LP.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        if hum then
            local t0 = tick()
            repeat task.wait(0.05) until tick() - t0 > 1.5 or (
                hum:GetState() ~= Enum.HumanoidStateType.Ragdoll and
                hum:GetState() ~= Enum.HumanoidStateType.FallingDown and
                hum:GetState() ~= Enum.HumanoidStateType.Physics
            )
        end
        StartPostTPDuel(side)
        -- استنى الأوتو دولز ينتهي قبل ما نعيد الكشف
        task.spawn(function()
            repeat task.wait(0.1) until not postTPActive
            task.wait(0.2)
            lastRagdollState = false
            duelTPPaused = false  -- أعد الكشف بعد ما ينتهي الأوتو دولز
        end)
    else
        duelTPPaused = false
    end
...
local function startDuelTP()
    ...
    duelTPConn = RunService.Heartbeat:Connect(function()
        ...
        if duelTPPaused then
            lastRagdollState = true  -- نخلّيها true عشان ما يعيد يشغّل
            return
        end
        local char = LP.Character
        if not char then return end
        ...
        local ragdollNow = false
        local st = hum:GetState()
        if st == Enum.HumanoidStateType.Ragdoll or
           st == Enum.HumanoidStateType.FallingDown or
           st == Enum.HumanoidStateType.Physics then
            ragdollNow = true
        end
        if ragdollNow and not lastRagdollState then
            task.spawn(onRagdollHit)
        end
        lastRagdollState = ragdollNow
    end)
...
RunService.Heartbeat:Connect(function(dt)
    ...
    -- Post-TP Duel يشتغل من كونكشنه الخاص (postTPConn) وما يحتاج هنا
end)
...
initWPParts()
ApplyLoadedState()
Notify("H2N V10.1 | TP:0.17+0.13s | Auto-TP on hit")
print("=" .. string.rep("=", 60))
print("📌 NORMAL AUTO DUEL (5 نقاط):")
print("✓ LEFT:  L1→L2→L1→R1→R2")
print("✓ RIGHT: R1→R2→R1→L1→L2")
print("=" .. string.rep("=", 60))
print("📌 POST-HIT AUTO DUEL (4 نقاط - يتكرر مع كل ضربة):")
print("✓ LEFT:  L2→L1→R1→R2")
print("✓ RIGHT: R2→R1→L1→L2")
print("=" .. string.rep("=", 60))
print("📌 TP DELAYS: نقطة1=0.17s | نقطة2=0.13s")
print("📌 لما تنضرب (RAGDOLL TP مفعل):")
print("  - أوتو دولز عادي → يوقف + TP + مسار 4 نقاط")
print("  - مسار 4 نقاط    → يعيد TP + مسار 4 نقاط")
print("  - مش مفعل أوتو   → TP + مسار 4 نقاط تلقائي")
print("=" .. string.rep("=", 60))
print("📌 G=AutoLeft | H=AutoRight | R=RagdollTP | K=AntiRagdoll")
print("=" .. string.rep("=", 60))
