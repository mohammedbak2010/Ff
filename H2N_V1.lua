-- H2N V1 | Ultimate Edition | Auto Duel No Delay | No Home Set | Fixed Toggles
...
-- مسارات الـ TP (4 نقاط تبدأ من النقطة الثانية)
local LEFT_TP_ROUTE  = {"L2", "L1", "R1", "R2"}
local RIGHT_TP_ROUTE = {"R2", "R1", "L1", "L2"}
...
-- ====== AUTO DUEL (مسافة كشف 0.6، بدون أي انتظار) ======
local aplConn, aprConn = nil, nil
local aplPhase, aprPhase = 1, 1
-- هل الأوتو دولز شتغل بسبب TP؟ (يستخدم 4 نقاط)
local aplTPMode = false
local aprTPMode = false
...
-- مسافة الكشف: 0.6 (وصول دقيق)
local function MoveToPoint(hrp, targetPos, speed)
    if not hrp or not targetPos then return true end
    -- نحسب المسافة على X و Z فقط (نتجاهل Y) عشان Float ما يأثر على الوصول للنقطة
...
local function updateAutoPlayLeft()
    ...
    local route = aplTPMode and LEFT_TP_ROUTE or LEFT_ROUTE
    local target = route[aplPhase]
    ...
    if reached then
        aplPhase = aplPhase + 1
        if aplPhase > #route then StopAutoPlayLeft() end
    end
end

-- تحديث Auto Play Right
local function updateAutoPlayRight()
    ...
    local h = getHRP2()
    if not h then return end
    local route = aprTPMode and RIGHT_TP_ROUTE or RIGHT_ROUTE
    local target = route[aprPhase]
    ...
    if reached then
        aprPhase = aprPhase + 1
        if aprPhase > #route then StopAutoPlayRight() end
    end
end

-- زر موحد: بدون Ragdoll TP يشغل 5 نقاط، مع Ragdoll TP ينتظر الضربة
local function StartAutoPlayLeft()
    if State.AutoPlayLeft then StopAutoPlayLeft(); return end
    if State.AutoPlayRight then StopAutoPlayRight() end
    State.AutoPlayLeft = true
    aplTPMode = false  -- عادي دايماً لما تشغله يدوي
    aplPhase = 1
...
local function StartAutoPlayRight()
    if State.AutoPlayRight then StopAutoPlayRight(); return end
    if State.AutoPlayLeft then StopAutoPlayLeft() end
    State.AutoPlayRight = true
    aprTPMode = false  -- عادي دايماً لما تشغله يدوي
    aprPhase = 1
    if Hum then Hum.AutoRotate = false end
    if aprConn then aprConn:Disconnect(); aprConn = nil end
...
local function doTeleportSequence(side)
    ...
    if hrp then hrp.CFrame = CFrame.new(points[1]) end
    Notify("⚡ TP → " .. (side == "LEFT" and "L1" or "R1"))
    task.wait(0.2)

    -- تحقق إن Ragdoll TP لسه مفعل
    if not State.DuelTP then duelTPInProgress = false; return end

    -- خطوة 2: TP لـ L2/R2
    char = LP.Character
    hrp = char and char:FindFirstChild("HumanoidRootPart")
    if hrp then hrp.CFrame = CFrame.new(points[2]) end
    Notify("⚡ TP → " .. (side == "LEFT" and "L2" or "R2"))
    task.wait(0.15)

    if not State.DuelTP then duelTPInProgress = false; return end

    -- خطوة 3: شغل auto duel بـ 4 نقاط (TP mode) من البداية
    if State.AutoPlayLeft then StopAutoPlayLeft() end
    if State.AutoPlayRight then StopAutoPlayRight() end
    ...
    if side == "LEFT" then
        ...
        if aprConn then aprConn:Disconnect(); aprConn = nil end
        aprConn = RunService.Heartbeat:Connect(updateAutoPlayRight)
        Notify("▶ TP Route: R2→R1→L1→L2")
    end

    -- cooldown ثم افتح الباب لضربة جديدة
    DuelTP.LastTPTime = tick()
    task.wait(DuelTP.Cooldown)
    duelTPInProgress = false
end

local function onRagdollHit()
    if not State.DuelTP then return end
    if duelTPInProgress then return end
    local now = tick()
    if now - DuelTP.LastTPTime < DuelTP.Cooldown then return end
    duelTPInProgress = true
    task.spawn(doTeleportSequence, DuelTP.EnemySide)
end
...
local function MakeToggle(parent, text, order, cb, getState)
    ...
    -- تحديث تلقائي للزر عشان يعكس الحالة الفعلية دايماً
...
MakeToggle(combat, "Auto Duel", ci,
    function(s)
        if s then
            local side = DuelTP.EnemySide or "LEFT"
            if side == "LEFT" then StartAutoPlayLeft() else StartAutoPlayRight() end
        else
            if State.AutoPlayLeft then StopAutoPlayLeft() end
            if State.AutoPlayRight then StopAutoPlayRight() end
        end
    end,
    function() return State.AutoPlayLeft or State.AutoPlayRight end
); ci = ci + 1
...
local sideNames = {"AUTO DUEL", "AUTO TRACK", "RAGDOLL TP", "FLOAT"}
...
-- زر AUTO DUEL الموحد (يحل محل LEFT و RIGHT)
CreateSideButton(
    "AUTO DUEL",
    "left",
    0,
...
initWPParts()
ApplyLoadedState()
Notify("H2N V1 | No Delay Auto Duel | No Home Set | Fixed Toggles")
print("=" .. string.rep("=", 60))
