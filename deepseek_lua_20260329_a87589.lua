-- H2N V9.9 | Ultimate Edition | Full Complete Script
...
-- ====== الإعدادات ======
local SPEED_BOOST_VALUE   = 60
local BRAINROT_SLOW_SPEED = 16
...
local function SaveConfig()
    local cfg = {
        SPEED_BOOST_VALUE = SPEED_BOOST_VALUE,
        BRAINROT_SLOW_SPEED = BRAINROT_SLOW_SPEED,
        ...
        Keys = {
            SpeedKey=Keys.SpeedKey.Name, InfJump=Keys.InfJump.Name,
            AutoPlayLeft=Keys.AutoPlayLeft.Name, AutoPlayRight=Keys.AutoPlayRight.Name,
            AutoTrack=Keys.AutoTrack.Name, DuelTP=Keys.DuelTP.Name, AntiRagdoll=Keys.AntiRagdoll.Name
        }
    }
...
local function LoadConfig()
    ...
    SPEED_BOOST_VALUE = data.SPEED_BOOST_VALUE or SPEED_BOOST_VALUE
    BRAINROT_SLOW_SPEED = data.BRAINROT_SLOW_SPEED or BRAINROT_SLOW_SPEED
    ...
    if data.Keys then for k,v in pairs(data.Keys) do local e=Enum.KeyCode[v]; if e and Keys[k] then Keys[k]=e end end end
end
LoadConfig()
task.spawn(function() while true do task.wait(10); pcall(SaveConfig) end end)
...
-- ====== SPEED BOOST ======
local lastVelX, lastVelZ = 0, 0
local speedJumpCooldown = 0
local function StartSpeedBoost() State.SpeedBoost = true; Notify("Speed Boost ON") end
local function StopSpeedBoost() State.SpeedBoost = false; Notify("Speed Boost OFF") end
...
local function startFloat()
    ...
    if carryingBrainrot then Notify("Cannot Float with Brainrot!"); return end
    FloatOriginalY = hrp.Position.Y
    State.FloatEnabled = true
    FloatConn = RunService.Heartbeat:Connect(function()
        if not State.FloatEnabled or not HRP then return end
        if carryingBrainrot then stopFloat(); Notify("Float disabled: Brainrot!"); return end
...
MakeNumberBox(sTab, "Boost Speed", SPEED_BOOST_VALUE, si, function(v) SPEED_BOOST_VALUE = math.clamp(v,16,200) end); si = si + 1
MakeNumberBox(sTab, "Brainrot Slow Speed", BRAINROT_SLOW_SPEED, si, function(v) BRAINROT_SLOW_SPEED = math.clamp(v,10,30); Notify("Brainrot Slow = "..BRAINROT_SLOW_SPEED) end); si = si + 1
...
CreateSideButton("ANTI RAGDOLL", "left", 2, function() return antiRagdollEnabled end, StartAntiRagdoll, StopAntiRagdoll)
...
RunService.Heartbeat:Connect(function(dt)
    -- Speed System with Brainrot detection
    if Hum and Char and HRP then
        ...
        if State.SpeedBoost then
            if carryingBrainrot then
                targetSpeed = BRAINROT_SLOW_SPEED
            else
                targetSpeed = SPEED_BOOST_VALUE
            end
        end
        ...
        if flatV > targetSpeed + 10 then
            HRP.AssemblyLinearVelocity = Vector3.new(v.X * 0.95, v.Y, v.Z * 0.95)
        end
    end
...
-- ====== INIT ======
initWPParts()
Notify("H2N V9.9 | Ultimate Edition")
Notify("✓ Auto Duel Fixed - Full path movement")
Notify("✓ Speed Boost with Brainrot detection (slows to " .. BRAINROT_SLOW_SPEED .. " when carrying Brainrot)")
Notify("✓ Ragdoll TP: Stand 1.5s in base to set home")

print("=" .. string.rep("=", 50))
print("📌 H2N V9.9 ULTIMATE EDITION - ALL FEATURES:")
print("✓ Combat: Auto Track, Auto Play Left/Right (Full path), Auto Grab, Anti Sentry, Spin Body")
print("✓ Protect: Anti Ragdoll, Infinite Jump, RAGDOLL TP, FLOAT")
print("✓ Visual: ESP, Xray Base, Customizable Side Buttons")
print("✓ Settings: Speed Boost with Brainrot detection (slows to " .. BRAINROT_SLOW_SPEED .. ")")
print("✓ Keybinds: V=Speed, J=Jump, G=AutoLeft, H=AutoRight, T=Track, R=RagdollTP, K=AntiRagdoll")
print("=" .. string.rep("=", 50))