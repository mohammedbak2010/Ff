-- H2N V9.9 | Ultimate Edition | Auto Duel No Delay | No Home Set | Fixed Menu Toggles
...
-- ====== RAGDOLL TP ======
local DuelTP = {
    Enabled = false, MyHome = nil, EnemySide = "RIGHT", HomeSet = true,
    LastTPTime = 0, Cooldown = 2,
...
local function StartAntiRagdoll()
    ...
    if antiRagdollMenuBtn then antiRagdollMenuBtn:Update() end
end
...
local function SaveEverything()
    local cfg = {
        ...
        DuelTP = {MyHome = DuelTP.MyHome, EnemySide = DuelTP.EnemySide},
        ...
        State = {
            ...
            AutoPlayLeft = State.AutoPlayLeft, AutoPlayRight = State.AutoPlayRight,
        }
    }
...
local function LoadEverything()
    ...
    if data.DuelTP then 
        DuelTP.MyHome = data.DuelTP.MyHome
        DuelTP.EnemySide = data.DuelTP.EnemySide or "RIGHT"
        DuelTP.HomeSet = true
    end
...
local function startFloat()
    ...
    Notify("Float ON")
    if floatMenuBtn then floatMenuBtn:Update() end
end
...
local function StopAutoPlayLeft()
    ...
    if autoLeftMenuBtn then autoLeftMenuBtn:Update() end
end
...
local function StopAutoPlayRight()
    ...
    if autoRightMenuBtn then autoRightMenuBtn:Update() end
end
...
local function startDuelTP()
    ...
    Notify("RAGDOLL TP Enabled")
    if duelTPMenuBtn then duelTPMenuBtn:Update() end
end
...
local function StartAutoTrack()
    ...
    if autoTrackMenuBtn then autoTrackMenuBtn:Update() end
end
...
local function StartInfiniteJump()
    ...
    if infJumpMenuBtn then infJumpMenuBtn:Update() end
end
...
local function StartAutoGrab()
    ...
    Notify("Auto Grab ON")
    if autoGrabMenuBtn then autoGrabMenuBtn:Update() end
end
...
local function StartXrayBase()
    ...
    Notify("Xray Base ON")
    if xrayMenuBtn then xrayMenuBtn:Update() end
end
...
local function StartESP() 
    State.ESP = true
    Notify("ESP ON")
    if espMenuBtn then espMenuBtn:Update() end
end
...
local function StartAntiSentry() 
    ...
    if antiSentryMenuBtn then antiSentryMenuBtn:Update() end
end
...
local function StartSpinBody()
    ...
    Notify("Spin Body ON")
    if spinBodyMenuBtn then spinBodyMenuBtn:Update() end
end
...
-- ====== MAKE TOGGLE (Menu Buttons with Update Function) ======
...
-- متغيرات لأزرار القائمة
local autoTrackMenuBtn, autoGrabMenuBtn, antiSentryMenuBtn, spinBodyMenuBtn = nil, nil, nil, nil
local autoLeftMenuBtn, autoRightMenuBtn = nil, nil

autoTrackMenuBtn = MakeToggle(combat, "Auto Track", ci, function(s) if s then StartAutoTrack() else StopAutoTrack() end end, function() return State.AutoTrack end); ci = ci + 1
...
autoLeftMenuBtn = MakeToggle(combat, "Auto Play Left", ci, function(s) if s then StartAutoPlayLeft() else StopAutoPlayLeft() end end, function() return State.AutoPlayLeft end); ci = ci + 1
autoRightMenuBtn = MakeToggle(combat, "Auto Play Right", ci, function(s) if s then StartAutoPlayRight() else StopAutoPlayRight() end end, function() return State.AutoPlayRight end); ci = ci + 1
autoGrabMenuBtn = MakeToggle(combat, "Auto Grab", ci, function(s) if s then StartAutoGrab() else StopAutoGrab() end end, function() return State.AutoGrab end); ci = ci + 1
...
antiSentryMenuBtn = MakeToggle(combat, "Anti Sentry", ci, function(s) if s then StartAntiSentry() else StopAntiSentry() end end, function() return State.AntiSentry end); ci = ci + 1
spinBodyMenuBtn = MakeToggle(combat, "Spin Body", ci, function(s) if s then StartSpinBody() else StopSpinBody() end end, function() return State.SpinBody end); ci = ci + 1

-- ====== PROTECT TAB ======
local pi = 0
local protect = tabFrames["Protect"]

local antiRagdollMenuBtn, infJumpMenuBtn, duelTPMenuBtn, floatMenuBtn = nil, nil, nil, nil

antiRagdollMenuBtn = MakeToggle(protect, "Anti Ragdoll", pi, function(s) if s then StartAntiRagdoll() else StopAntiRagdoll() end end, function() return antiRagdollEnabled end); pi = pi + 1
infJumpMenuBtn = MakeToggle(protect, "Infinite Jump", pi, function(s) if s then StartInfiniteJump() else StopInfiniteJump() end end, function() return State.InfiniteJump end); pi = pi + 1
duelTPMenuBtn = MakeToggle(protect, "RAGDOLL TP", pi, function(s) if s then startDuelTP() else stopDuelTP() end end, function() return State.DuelTP end); pi = pi + 1
floatMenuBtn = MakeToggle(protect, "FLOAT", pi, function(s) if s then startFloat() else stopFloat() end end, function() return State.FloatEnabled end); pi = pi + 1

-- ====== VISUAL TAB ======
local vi = 0
local visual = tabFrames["Visual"]

local espMenuBtn, xrayMenuBtn = nil, nil

espMenuBtn = MakeToggle(visual, "ESP", vi, function(s) if s then StartESP() else StopESP() end end, function() return State.ESP end); vi = vi + 1
xrayMenuBtn = MakeToggle(visual, "Xray Base", vi, function(s) if s then StartXrayBase() else StopXrayBase() end end, function() return State.XrayBase end); vi = vi + 1

-- خاصية إخفاء جميع الأزرار الجانبية
...
local function ApplyLoadedState()
    if State.AutoTrack then StartAutoTrack() else if autoTrackMenuBtn then autoTrackMenuBtn:Update() end end
    if State.AutoGrab then StartAutoGrab() else if autoGrabMenuBtn then autoGrabMenuBtn:Update() end end
    if State.AntiSentry then StartAntiSentry() else if antiSentryMenuBtn then antiSentryMenuBtn:Update() end end
    if State.SpinBody then StartSpinBody() else if spinBodyMenuBtn then spinBodyMenuBtn:Update() end end
    if State.AntiRagdoll then StartAntiRagdoll() else if antiRagdollMenuBtn then antiRagdollMenuBtn:Update() end end
    if State.InfiniteJump then StartInfiniteJump() else if infJumpMenuBtn then infJumpMenuBtn:Update() end end
    if State.DuelTP then startDuelTP() else if duelTPMenuBtn then duelTPMenuBtn:Update() end end
    if State.FloatEnabled then startFloat() else if floatMenuBtn then floatMenuBtn:Update() end end
    if State.XrayBase then StartXrayBase() else if xrayMenuBtn then xrayMenuBtn:Update() end end
    if State.ESP then StartESP() else if espMenuBtn then espMenuBtn:Update() end end
    if State.AutoPlayLeft then StartAutoPlayLeft() else if autoLeftMenuBtn then autoLeftMenuBtn:Update() end end
    if State.AutoPlayRight then StartAutoPlayRight() else if autoRightMenuBtn then autoRightMenuBtn:Update() end end
end

-- ====== INIT ======
initWPParts()
ApplyLoadedState()
Notify("H2N V9.9 | No Delay Auto Duel | No Home Set | Fixed Menu Toggles")
print("=" .. string.rep("=", 60))
print("📌 AUTO DUEL PATHS (0.6 detection, No Delay):")
print("✓ LEFT: L1 → L2 → L1 → R1 → R2")
print("✓ RIGHT: R1 → R2 → R1 → L1 → L2")
print("=" .. string.rep("=", 60))
print("📌 CONTROLS:")
print("✓ Side Buttons: Click to toggle ON/OFF")
print("✓ Menu Button: H2N (top center)")
print("✓ Keybinds: G=AutoLeft, H=AutoRight, T=Track, J=Jump, R=RagdollTP, K=AntiRagdoll")
print("=" .. string.rep("=", 60))