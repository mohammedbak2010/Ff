-- H2N V9.8 | FINAL | New Steal Bar Design | Fixed Auto Grab | New Ragdoll TP System | Smooth Auto Duel
...
local function StartAutoPlayLeft()
    StopAutoPlayLeft(); StopAutoPlayRight()
    State.AutoPlayLeft = true; aplPhase = 1; aplWaiting = false
...
local function StartAutoPlayRight()
    StopAutoPlayRight(); StopAutoPlayLeft()
    State.AutoPlayRight = true; aprPhase = 1; aprWaiting = false
    if Hum then Hum.AutoRotate = false end
...
-- Start from specific phase (used after TP)
local function StartAutoPlayFromPhase(side, startPhase)
    if side == "LEFT" then
        StopAutoPlayLeft()
...
local function teleportSequence(side)
    ...
    -- Teleport to first point (L1/R1)
    hrp.CFrame = CFrame.new(points[1])
    task.wait(0.2)
    
    -- Teleport to second point (L2/R2)
...
local function onRagdollHit()
    ...
    if success then
        -- Start Auto Duel from phase 2 (starting from L2 or R2)
        if side == "LEFT" then
            StartAutoPlayFromPhase("LEFT", 2)
        else
            StartAutoPlayFromPhase("RIGHT", 2)
        end
        Notify("⚡ Ragdoll TP → " .. (side == "LEFT" and "LEFT SIDE" or "RIGHT SIDE"))
        return true
    end
    return false
end

-- Ragdoll TP connection
...
local function UpdateGrabBar(pct)
    ...
    if grabBarRef.radiusLbl then grabBarRef.radiusLbl.Text = GRAB_RADIUS.."m" end
    if grabBarRef.rateLbl then grabBarRef.rateLbl.Text = string.format("%.2f", GRAB_RATE).."s" end
end

local function updateAnimalCache()
                ...
                if podiums then
                    for _, podium in ipairs(podiums:GetChildren()) do
                        if podium:IsA("Model") and podium:FindFirstChild("Base") then
                            local animal = podium:FindFirstChild("Animal")
                            if animal and animal:IsA("Model") then
                                local root = animal:FindFirstChild("HumanoidRootPart") or animal:FindFirstChildWhichIsA("BasePart")
                                if root then
...
local function stealAnimal(animal)
    ...
    if dist > GRAB_RADIUS + 2 then return false end
    
    -- Try to grab by moving to animal
    ...
    -- Try to equip and use bat
    local bat = LP.Backpack:FindFirstChild("Bat") or char:FindFirstChild("Bat")
    if bat then
...
local function StartAutoGrab()
    ...
    grabMainConn = RunService.Heartbeat:Connect(function(dt)
        if not State.AutoGrab or not HRP then return end
        
        local best, bestDist = nil, GRAB_RADIUS
        for _, a in ipairs(allAnimalsCache) do
            if a.position then
                local d = (HRP.Position - a.position).Magnitude
                if d < bestDist then
                    bestDist = d
                    best = a
                end
        ...
        if best and best.animal then
            grabTimer = grabTimer + dt
            UpdateGrabBar((grabTimer / GRAB_RATE) * 100)
            
            if grabTimer >= GRAB_RATE then
                grabTimer = 0
                local success = stealAnimal(best.animal)
                if success then
                    Notify("✨ Stole " .. (best.animal.Name or "Animal") .. "!")
                end
                updateAnimalCache()
            end
        else
...
local function StopESP() State.ESP = false; ClearESP(); SaveEverything() end
...
local function findSentryTarget()
    ...
    for _, obj in pairs(workspace:GetChildren()) do
        if obj.Name:find("Sentry") and not obj.Name:lower():find("bullet") then
            ...
            if ownerId and tonumber(ownerId) == LP.UserId then else
...
local function getWeapon() local char = LP.Character; if not char then return nil end; return LP.Backpack:FindFirstChild("Bat") or char:FindFirstChild("Bat") end
    ...
    task.spawn(function()
        while State.AntiSentry do
            if antiSentryTarget and antiSentryTarget.Parent == workspace then
                moveSentry(antiSentryTarget); attackSentry()
...
local function StopAntiSentry() State.AntiSentry = false; antiSentryTarget = nil; SaveEverything() end
...
local function StopSpinBody() State.SpinBody = false; if spinForce then spinForce:Destroy(); spinForce = nil end; SaveEverything() end
...
local function StopSpeedBoost() State.SpeedBoost = false; if speedBoostConn then speedBoostConn:Disconnect(); speedBoostConn = nil end; SaveEverything() end
...
-- ====== STEAL BAR (NEW DESIGN) ======
local stealBarFrame = Instance.new("Frame", gui)
stealBarFrame.Name = "StealBar"
stealBarFrame.Size = UDim2.new(0, 380, 0, 48)
stealBarFrame.Position = UDim2.new(0.5, -190, 1, -65)
stealBarFrame.BackgroundColor3 = Color3.fromRGB(8, 0, 8)
...
sbStroke.Color = Color3.fromRGB(255, 50, 100)
...
-- Left icon section
local iconFrame = Instance.new("Frame", stealBarFrame)
iconFrame.Size = UDim2.new(0, 48, 1, 0)
iconFrame.BackgroundColor3 = Color3.fromRGB(255, 30, 80)
iconFrame.BackgroundTransparency = 0.2
iconFrame.ZIndex = 9
Instance.new("UICorner", iconFrame).CornerRadius = UDim.new(0, 14)

local icon = Instance.new("TextLabel", iconFrame)
...
sbBG.Size = UDim2.new(0, 200, 0, 24)
sbBG.Position = UDim2.new(0, 120, 0.5, -12)
sbBG.BackgroundColor3 = Color3.fromRGB(20, 0, 20)
sbBG.ZIndex = 9
...
sbFill.BackgroundColor3 = Color3.fromRGB(255, 50, 100)
sbFill.ZIndex = 10
Instance.new("UICorner", sbFill).CornerRadius = UDim.new(0, 12)

-- Percentage
local sbPct = Instance.new("TextLabel", stealBarFrame)
sbPct.Size = UDim2.new(0, 50, 1, 0)
sbPct.Position = UDim2.new(1, -135, 0, 0)
...
sbRadius.Size = UDim2.new(0, 45, 1, 0)
sbRadius.Position = UDim2.new(1, -85, 0, 0)
sbRadius.BackgroundTransparency = 1
sbRadius.Text = GRAB_RADIUS.."m"
sbRadius.TextColor3 = Color3.fromRGB(255, 180, 80)
sbRadius.Font = Enum.Font.GothamBold
sbRadius.TextSize = 12
sbRadius.ZIndex = 9

-- Rate
local sbRate = Instance.new("TextLabel", stealBarFrame)
sbRate.Size = UDim2.new(0, 60, 1, 0)
sbRate.Position = UDim2.new(1, -40, 0, 0)
sbRate.BackgroundTransparency = 1
sbRate.Text = string.format("%.2f",GRAB_RATE).."s"
sbRate.TextColor3 = Color3.fromRGB(100, 255, 150)
...
MakeNumberBox(combat, "Grab Radius", GRAB_RADIUS, ci, function(v)
    ...
    if State.AutoGrab then
        StopAutoGrab(); task.wait(0.05); StartAutoGrab()
    end
end)
ci = ci + 1
...
MakeToggle(visual, "Hide All Side Btns", vi, function() return hideAllState end, function(s)
    hideAllState = s
...
for _, nm in ipairs(sideNames) do
    MakeToggle(visual, "Hide "..nm, vi, function() return sideHiddenMap[nm.."_individual"] == true end, function(s)
        sideHiddenMap[nm.."_individual"] = s
        for _, b in pairs(gui:GetChildren()) do
...
MakeToggle(visual, "Show Steal Bar", vi, function() return StealBarVisible end, function(s) 
    StealBarVisible = s
...
-- Side Button Size
local sideBtnSizeRow = Instance.new("Frame", visual)
sideBtnSizeRow.Size = UDim2.new(1, -10, 0, 40)
sideBtnSizeRow.Position = UDim2.new(0, 5, 0, vi * 44 + 4)
sideBtnSizeRow.BackgroundTransparency = 1
sideBtnSizeRow.ZIndex = 11

local sideBtnSizeLbl = Instance.new("TextLabel", sideBtnSizeRow)
sideBtnSizeLbl.Size = UDim2.new(0.7, 0, 1, 0)
sideBtnSizeLbl.Position = UDim2.new(0, 10, 0, 0)
sideBtnSizeLbl.BackgroundTransparency = 1
sideBtnSizeLbl.Text = "Side Button Size"
sideBtnSizeLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
sideBtnSizeLbl.Font = Enum.Font.GothamBold
sideBtnSizeLbl.TextSize = 13
sideBtnSizeLbl.TextXAlignment = Enum.TextXAlignment.Left
sideBtnSizeLbl.ZIndex = 12

local sideBtnSizeBox = Instance.new("TextBox", sideBtnSizeRow)
sideBtnSizeBox.Size = UDim2.new(0.25, 0, 0.7, 0)
sideBtnSizeBox.Position = UDim2.new(0.72, 0, 0.15, 0)
sideBtnSizeBox.BackgroundColor3 = Color3.fromRGB(10, 0, 0)
sideBtnSizeBox.Text = tostring(SideButtonSize)
sideBtnSizeBox.TextColor3 = Color3.fromRGB(255, 255, 255)
sideBtnSizeBox.Font = Enum.Font.GothamBold
sideBtnSizeBox.TextSize = 12
sideBtnSizeBox.ZIndex = 12
Instance.new("UICorner", sideBtnSizeBox).CornerRadius = UDim.new(0, 8)

sideBtnSizeBox.FocusLost:Connect(function()
    local n = tonumber(sideBtnSizeBox.Text)
    if n then
        SideButtonSize = math.clamp(n, 40, 150)
        sideBtnSizeBox.Text = tostring(SideButtonSize)
        SaveEverything()
        Notify("Restart GUI to apply new button size")
    else
        sideBtnSizeBox.Text = tostring(SideButtonSize)
    end
end)
...
-- Menu Size
local menuSizeRow = Instance.new("Frame", visual)
menuSizeRow.Size = UDim2.new(1, -10, 0, 40)
menuSizeRow.Position = UDim2.new(0, 5, 0, vi * 44 + 4)
menuSizeRow.BackgroundTransparency = 1
menuSizeRow.ZIndex = 11

local menuSizeLbl = Instance.new("TextLabel", menuSizeRow)
menuSizeLbl.Size = UDim2.new(0.7, 0, 1, 0)
menuSizeLbl.Position = UDim2.new(0, 10, 0, 0)
menuSizeLbl.BackgroundTransparency = 1
menuSizeLbl.Text = "Menu Width"
menuSizeLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
menuSizeLbl.Font = Enum.Font.GothamBold
menuSizeLbl.TextSize = 13
menuSizeLbl.TextXAlignment = Enum.TextXAlignment.Left
menuSizeLbl.ZIndex = 12

local menuWidthBox = Instance.new("TextBox", menuSizeRow)
menuWidthBox.Size = UDim2.new(0.25, 0, 0.7, 0)
menuWidthBox.Position = UDim2.new(0.72, 0, 0.15, 0)
menuWidthBox.BackgroundColor3 = Color3.fromRGB(10, 0, 0)
menuWidthBox.Text = tostring(menuW)
menuWidthBox.TextColor3 = Color3.fromRGB(255, 255, 255)
menuWidthBox.Font = Enum.Font.GothamBold
menuWidthBox.TextSize = 12
menuWidthBox.ZIndex = 12
Instance.new("UICorner", menuWidthBox).CornerRadius = UDim.new(0, 8)

menuWidthBox.FocusLost:Connect(function()
    local n = tonumber(menuWidthBox.Text)
    if n then
        menuW = math.clamp(n, 250, 600)
        menuWidthBox.Text = tostring(menuW)
        menu.Size = UDim2.new(0, menuW, 0, menuH)
        SaveEverything()
    else
...
local menuHeightRow = Instance.new("Frame", visual)
menuHeightRow.Size = UDim2.new(1, -10, 0, 40)
menuHeightRow.Position = UDim2.new(0, 5, 0, vi * 44 + 4)
menuHeightRow.BackgroundTransparency = 1
menuHeightRow.ZIndex = 11

local menuHeightLbl = Instance.new("TextLabel", menuHeightRow)
menuHeightLbl.Size = UDim2.new(0.7, 0, 1, 0)
menuHeightLbl.Position = UDim2.new(0, 10, 0, 0)
menuHeightLbl.BackgroundTransparency = 1
menuHeightLbl.Text = "Menu Height"
menuHeightLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
menuHeightLbl.Font = Enum.Font.GothamBold
menuHeightLbl.TextSize = 13
menuHeightLbl.TextXAlignment = Enum.TextXAlignment.Left
menuHeightLbl.ZIndex = 12

local menuHeightBox = Instance.new("TextBox", menuHeightRow)
menuHeightBox.Size = UDim2.new(0.25, 0, 0.7, 0)
menuHeightBox.Position = UDim2.new(0.72, 0, 0.15, 0)
menuHeightBox.BackgroundColor3 = Color3.fromRGB(10, 0, 0)
menuHeightBox.Text = tostring(menuH)
menuHeightBox.TextColor3 = Color3.fromRGB(255, 255, 255)
menuHeightBox.Font = Enum.Font.GothamBold
menuHeightBox.TextSize = 12
menuHeightBox.ZIndex = 12
Instance.new("UICorner", menuHeightBox).CornerRadius = UDim.new(0, 8)

menuHeightBox.FocusLost:Connect(function()
    local n = tonumber(menuHeightBox.Text)
    if n then
        menuH = math.clamp(n, 200, 500)
        menuHeightBox.Text = tostring(menuH)
        menu.Size = UDim2.new(0, menuW, 0, menuH)
        SaveEverything()
    else
...
do
    ...
    for _, wp in ipairs(WPS) do
        ...
        setBtn.MouseButton1Click:Connect(function()
            ...
            TP_POINTS = { LEFT = { L1, L2 }, RIGHT = { R1, R2 } }
...
local function CreateSideButton(text, side, index, getState, startFn, stopFn)
    ...
    btn.BackgroundColor3 = getState() and Color3.fromRGB(120, 0, 0) or Color3.fromRGB(40, 0, 0)
    btn.Active = true
    btn.ZIndex = 5
    ...
    dot.BackgroundColor3 = getState() and Color3.fromRGB(220, 0, 0) or Color3.fromRGB(80, 0, 0)
    dot.ZIndex = 6
...
-- ====== RAGDOLL TP SIDE BUTTON (FIXED - WITH DRAG + PRESS ANIMATION) ======
do
    ...
    mainF.BackgroundColor3 = State.DuelTP and Color3.fromRGB(120, 0, 0) or Color3.fromRGB(40, 0, 0)
    mainF.Active = true
    mainF.ZIndex = 5
    ...
    dot.Size = UDim2.new(0, 10, 0, 10)
    dot.Position = UDim2.new(0.5, -5, 1, -13)
    dot.BackgroundColor3 = State.DuelTP and Color3.fromRGB(220, 0, 0) or Color3.fromRGB(80, 0, 0)
    dot.ZIndex = 6
    Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
    ...
    -- ====== Tween انيميشن ======
    local twInfo = TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local twInfoOut = TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

    local function pressAnim()
        TweenService:Create(mainF, twInfo, {
    ...
    local function releaseAnim()
        TweenService:Create(mainF, twInfoOut, {
    ...
    -- ====== Update UI ======
    local function updateRagdollButtonUI()
        if State.DuelTP then
            TweenService:Create(mainF, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(120, 0, 0)}):Play()
            TweenService:Create(dot, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(220, 0, 0)}):Play()
            TweenService:Create(mStroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(255, 50, 50)}):Play()
        else
            TweenService:Create(mainF, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(40, 0, 0)}):Play()
            TweenService:Create(dot, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(80, 0, 0)}):Play()
            TweenService:Create(mStroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(220, 0, 0)}):Play()
        end
        dirLbl.Text = (DuelTP.EnemySide == "RIGHT") and "→ R" or "← L"
        arrowBtn.Text = (DuelTP.EnemySide == "RIGHT") and "R" or "L"
    end
    ...
    -- ====== Swap ======
    local function swapRagdollSide()
        if DuelTP.EnemySide == "RIGHT" then
            DuelTP.EnemySide = "LEFT"
        else
            DuelTP.EnemySide = "RIGHT"
        end
        updateRagdollButtonUI()
        if tpSV then
            tpSV.Text = (DuelTP.EnemySide == "LEFT") and "← LEFT" or "RIGHT →"
        end
        -- أنيميشن زر السواب
        TweenService:Create(arrowBtn, twInfo, {
            Size = UDim2.new(0, math.floor(arrowS * 0.8), 0, math.floor(arrowS * 0.8)),
        }):Play()
        arrowBtn.BackgroundColor3 = Color3.fromRGB(0, 80, 0)
        task.delay(0.15, function()
            TweenService:Create(arrowBtn, twInfoOut, {
                Size = UDim2.new(0, arrowS, 0, arrowS),
            }):Play()
            task.wait(0.12)
            if arrowBtn then arrowBtn.BackgroundColor3 = Color3.fromRGB(60, 20, 0) end
        end)
        SaveEverything()
    end

    -- ====== Toggle ======
    local function toggleRagdollTP()
        if State.DuelTP then
            stopDuelTP()
        else
            startDuelTP()
    ...
    -- ====== نظام السحب الصح (Threshold-based) ======
    ...
    container.InputBegan:Connect(function(input)
        ...
        -- تجاهل لو الضغط على زر السواب
        local mousePos = Vector2.new(input.Position.X, input.Position.Y)
        local ap = arrowBtn.AbsolutePosition
        local as = arrowBtn.AbsoluteSize
        if mousePos.X >= ap.X and mousePos.X <= ap.X + as.X and
           mousePos.Y >= ap.Y and mousePos.Y <= ap.Y + as.Y then
            return
        end
    ...
    UIS.InputChanged:Connect(function(input)
        ...
        if delta.Magnitude > DRAG_THRESHOLD then
            if not hasMoved then
                hasMoved = true
                releaseAnim()  -- إلغاء انيميشن الضغط عند السحب
            end
            local screenSize = gui.AbsoluteSize
            local newX = math.clamp(btnStart.X.Offset + delta.X, 0, screenSize.X - bSize)
            local newY = math.clamp(btnStart.Y.Offset + delta.Y, 0, screenSize.Y - bSize)
    ...
    -- زر السواب
    arrowBtn.MouseButton1Click:Connect(swapRagdollSide)
    arrowBtn.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.Touch then swapRagdollSide() end
    end)
...
print("H2N V9.8 - FINAL | New Steal Bar Design | Fixed Auto Grab | New Ragdoll TP System | Smooth Auto Duel | Side Swap Button Working")