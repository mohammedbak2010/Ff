-- H2N V9.8 | Full Features | Auto Save Everything (Auto Duel Fixed)
...
-- ====== الدوال المعدلة للتحديث (مع إضافة تأخير) ======
local function updateAutoPlayLeft()
    if not State.AutoPlayLeft then return end
    local h = getHRP2()
    if not h then return end
    
    -- تحديد السرعة بناءً على المرحلة: أول نقطتين (الذهاب) سرعة عالية، الباقي (العودة) سرعة منخفضة
    local spd = (aplPhase <= 2) and Settings.DUEL_APPROACH_SPD or Settings.DUEL_RETURN_SPD
    local target = LEFT_ROUTE[aplPhase]
    
    if target then
        local reached = MoveToPoint(h, getWP(target), spd)
        if reached then
            -- وصلنا إلى النقطة، ننتظر لحظة قصيرة ثم ننتقل للتالية
            task.wait(0.1)
            aplPhase = aplPhase + 1
...
print("H2N V9.8 - Full Features | Ready (Auto Duel Fixed)")
print("✓ All features saved every 5 seconds")
print("✓ Auto Duel Left/Right now moves smoothly through all points")
