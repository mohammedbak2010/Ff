-- H2N V9.8 | FINAL | Universal Executor Support | Fixed Ragdoll TP | Full Visual Settings | All Side Buttons Draggable
...
local function hasWriteFile()
    return pcall(function() writefile("test.txt", "test") end) and pcall(function() delfile("test.txt") end)
end

local function hasReadFile()
    return pcall(function() readfile("test.txt") end) or true
end
...
local function loadFromFile(path)
    if hasReadFile() and isfile and pcall(isfile, path) and isfile(path) then
        return pcall(function() return readfile(path) end) and readfile(path) or nil
    end
    return nil
end
...
local function SaveEverything()
    ...
    saveToFile(CONFIG_FILE, HttpService:JSONEncode(cfg))
end

local function LoadEverything()
    local dataStr = loadFromFile(CONFIG_FILE)
    if dataStr then
        pcall(function()
...
-- إخفاء كل زر على حدة (UNWALK تمت إزالته من القائمة)
...
-- ====== زر RAGDOLL TP الجانبي (مع زر SWAP يعمل بشكل صحيح) ======
do
    ...
    -- دالة تحديث واجهة زر RAGDOLL TP
    ...
    -- دالة تبديل الاتجاه (SWAP)
    local function swapRagdollSide()
        local newSide = (DuelTP.EnemySide == "RIGHT") and "LEFT" or "RIGHT"
        DuelTP.EnemySide = newSide
        ...
        Notify("Ragdoll TP: " .. (DuelTP.EnemySide == "LEFT" and "← LEFT" or "RIGHT →"))
        SaveEverything()
    end

    -- دالة تشغيل/إيقاف RAGDOLL TP
    ...
    -- الزر الرئيسي لتشغيل/إيقاف RAGDOLL TP
...
print("H2N V9.8 - FINAL | Universal Executor Support | Unwalk removed from side buttons | All fixes applied")