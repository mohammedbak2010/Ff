-- H2N V9.8 | Final Working | Advanced Anti-Ragdoll | Auto Taunt on Win | Fixed Auto Track
...
-- ====== AUTO TAUNT (من Nine Hub) ======
local function sendTauntMessage(msg)
    msg = msg or "/H2N"
    pcall(function()
...
-- ====== ADVANCED ANTI RAGDOLL (من Kawatan Duels) ======
...
local function updateHomeZone(dt)
    ...
    if newZone and DuelTP.TimeInZone >= DuelTP.StandDuration and not DuelTP.HomeSet then
        ...
        DuelTP.HomeSet = true
        DuelTP.EnemySide = (DuelTP.MyHome == "LEFT") and "RIGHT" or "LEFT"
        if Notify then Notify("🏠 Home: " .. (DuelTP.MyHome == "LEFT" and "LEFT" or "RIGHT")) end
    end
end
...
-- ====== AUTO TRACK (مصلح - مع توجيه الرأس) ======
local trackConn = nil
local trackBodyGyro = nil

local function GetClosestPlayer()
    if not HRP then return nil, nil
...
MakeToggle(protect, "Anti Ragdoll", pi, function(s) if s then StartAntiRagdoll() else StopAntiRagdoll() end end, function() return antiRagdollMode ~= nil end); pi = pi + 1
...
local sideNames = {"AUTO PLAY LEFT", "SPEED BOOST", "AUTO PLAY RIGHT", "AUTO TRACK", "RAGDOLL TP", "FLOAT"}
...
initWPParts()
Notify("H2N V9.8 | Advanced Anti-Ragdoll | Auto Taunt on Win | Fixed Auto Track")