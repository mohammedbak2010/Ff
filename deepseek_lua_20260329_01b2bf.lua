-- H2N V9.8 | Final Working | All Features Fixed + Anti Ragdoll Side Button
...
local DUEL_APPROACH_SPD   = 60   -- سرعة الذهاب
local DUEL_RETURN_SPD     = 29   -- سرعة الرجوع
...
local STAND_POINTS = {
    LEFT = Vector3.new(-466, -6, 114),
    RIGHT = Vector3.new(-466, -6, 6),
}
    ...
    antiRagdollConn = RunService.Heartbeat:Connect(function()
        ...
        if state == Enum.HumanoidStateType.Ragdoll or 
           ...
           state == Enum.HumanoidStateType.Physics then
            ...
            if root then
                -- منع النوك باك الرأسي، الحفاظ على الحركة الأفقية
        ...
        -- إصلاح الـ Motor6D إن وجدت معطلة
...
RunService.Heartbeat:Connect(function()
    if carryingBrainrot and not lastBrainrotState then
        sendChatMessage()
    end
...
local function StartSpeedBoost() State.SpeedBoost = true end
local function StopSpeedBoost() State.SpeedBoost = false end
...
-- ====== AUTO DUEL (السرعات الصحيحة مع تأخير عند الوصول) ======
local aplConn, aprConn = nil, nil
local aplPhase, aprPhase = 1, 1

-- المسار: L1 -> L2 -> L1 -> R1 -> R2
local LEFT_ROUTE  = {"L1","L2","L1","R1","R2"}
-- المسار: R1 -> R2 -> R1 -> L1 -> L2
...
local function updateHomeZone(dt)
    ...
    if newZone == DuelTP.CurrentZone then
        if newZone and not DuelTP.HomeSet then
            DuelTP.TimeInZone = DuelTP.TimeInZone + dt
        end
    else
...
local sideNames = {"AUTO PLAY LEFT", "SPEED BOOST", "AUTO PLAY RIGHT", "AUTO TRACK", "RAGDOLL TP", "FLOAT", "ANTI RAGDOLL"}
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if Hum and Char and HRP then
        ...
        if not (v.Y > 5 or speedJumpCooldown > 0) then
            ...
            if flatMag > 350 then
                HRP.AssemblyLinearVelocity = Vector3.new(v.X * 0.4, v.Y, v.Z * 0.4)
            else
                local targetSpeed = 16
                if State.SpeedBoost and not carryingBrainrot then
                ...
                if md.Magnitude > 0.05 and flatV < targetSpeed - 5 then
                    local tX = md.X * targetSpeed
                    local tZ = md.Z * targetSpeed
...
initWPParts()
Notify("H2N V9.8 | All Features Working | Anti Ragdoll Side Button Added | Auto Duel Speed Fixed")