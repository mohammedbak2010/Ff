...
-- ====== ENHANCED ANTI RAGDOLL (دمج من أفضل السكريبتات) ======
local antiRagdollConn = nil
local antiRagdollActive = false
local antiRagdollMode = "v1"

-- Anti Ragdoll المتقدم
local function setupAdvancedAntiRagdoll()
...
local function teleportSequence(side)
    ...
    -- النقل للنقطة الأولى (L1/R1) - وقوف 0.2 ثانية
    hrp.CFrame = CFrame.new(points[1])
    task.wait(0.2)

    -- النقل للنقطة الثانية (L2/R2) - وقوف 0.15 ثانية
...
-- الأزرار الجانبية الأساسية (كلها قابلة للسحب)
...
-- ====== زر RAGDOLL TP الجانبي (مع زر L/R الصغير) ======
do
...
-- ====== زر FLOAT الجانبي المنفصل (قابل للسحب مثل باقي الأزرار) ======
do
    local bSize = SideButtonSize

    local container = Instance.new("Frame", gui)
    container.Name = "SideButton"
    container:SetAttribute("ID", "FLOAT")
    container.Size = UDim2.new(0, bSize, 0, bSize)
    container.BackgroundTransparency = 1
    container.Active = true
    container.ZIndex = 5
    container.Visible = not (sideHiddenMap["FLOAT_individual"] == true)

    local sp = ButtonPositions["FLOAT"]
    if sp then
        container.Position = UDim2.new(sp.X, sp.XO, sp.Y, sp.YO)
    else
        container.Position = UDim2.new(1, -(bSize + 10), 0.22 + 3 * 0.19, 0)
    end

    -- نظام السحب
    local pressing, hasMoved, dragStart, btnStart, activeInput = false, false, nil, nil, nil
    local activeTouchForDragF = nil

    container.InputBegan:Connect(function(input)
        local t = input.UserInputType
        if t ~= Enum.UserInputType.Touch and t ~= Enum.UserInputType.MouseButton1 then return end
        if activeTouchForDragF ~= nil then return end
        if pressing then return end
        pressing = true
        hasMoved = false
        activeInput = input
        activeTouchForDragF = input
        dragStart = input.Position
        btnStart = container.Position
    end)
    ...
    container.InputEnded:Connect(function(input)
        ...
        pressing = false
        activeInput = nil
        activeTouchForDragF = nil

        if hasMoved then
            local p = container.Position
            ButtonPositions["FLOAT"] = { X = p.X.Scale, XO = p.X.Offset, Y = p.Y.Scale, YO = p.Y.Offset }
            SaveEverything()
        end
        hasMoved = false
        dragStart = nil
    ...
    local mainF = Instance.new("Frame", container)
    mainF.Size = UDim2.new(1, 0, 1, 0)
    mainF.BackgroundColor3 = State.FloatEnabled and Color3.fromRGB(120, 0, 0) or Color3.fromRGB(40, 0, 0)
    mainF.Active = true
    mainF.ZIndex = 5
    Instance.new("UICorner", mainF).CornerRadius = UDim.new(0, 14)
    local mStroke = Instance.new("UIStroke", mainF)
    ...
    local nameLbl = Instance.new("TextLabel", mainF)
    nameLbl.Size = UDim2.new(1, -4, 0.7, 0)
    nameLbl.Position = UDim2.new(0, 2, 0, 12)
    nameLbl.BackgroundTransparency = 1
    nameLbl.Text = "FLOAT"
    nameLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
    nameLbl.Font = Enum.Font.GothamBold
    ...
    dot.Size = UDim2.new(0, 10, 0, 10)
    dot.Position = UDim2.new(0.5, -5, 1, -13)
    dot.BackgroundColor3 = State.FloatEnabled and Color3.fromRGB(220, 0, 0) or Color3.fromRGB(80, 0, 0)
    dot.ZIndex = 6
    Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
    ...
    mainClick.MouseButton1Click:Connect(toggleFloat)
    mainClick.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.Touch then
            toggleFloat()
        end
...
print("H2N V9.8 - Fixed | Enhanced AntiRagdoll | Ragdoll TP stops: L1/R1=0.2, L2/R2=0.15 | All side buttons draggable | FLOAT side button added")