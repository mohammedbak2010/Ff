-- H2N V9.9 | Ultimate Edition | Auto Duel No Delay | No Home Set | Fixed Toggles + TP Delay
...
-- ====== RAGDOLL TP (مع تأخير بين النقاط) ======
local DuelTP = {
    Enabled = false, MyHome = nil, EnemySide = "RIGHT", HomeSet = true,
    LastTPTime = 0, Cooldown = 2,
    TPSequence = TP_SEQUENCE,
    TP_DELAY_1 = 0.2,   -- تأخير بعد أول نقطة (L1 أو R1)
    TP_DELAY_2 = 0.15,  -- تأخير بعد ثاني نقطة (L2 أو R2)
}
...
-- تحديث Auto Play Left (بدون أي انتظار)
local function updateAutoPlayLeft()
    ...
    if reached then
        -- الانتقال فوراً للنقطة التالية بدون أي انتظار
        aplPhase = aplPhase + 1
...
-- تحديث Auto Play Right (بدون أي انتظار)
...
-- ====== RAGDOLL TP (مع تأخير بين النقاط - بدون تأثير على Auto Play العادي) ======
...
-- دالة التليبوست مع التأخير (هذا هو التعديل المطلوب)
local function teleportSequenceWithDelay(side)
    local now = tick()
    ...
    -- النقطة الأولى (L1 أو R1)
    hrp.CFrame = CFrame.new(points[1])
    task.wait(DuelTP.TP_DELAY_1)  -- انتظار 0.2 ثانية
    
    -- النقطة الثانية (L2 أو R2)
    hrp.CFrame = CFrame.new(points[2])
    task.wait(DuelTP.TP_DELAY_2)  -- انتظار 0.15 ثانية
...
local function onRagdollHit()
    ...
    local success, finalPoint = teleportSequenceWithDelay(side)
...
local function startDuelTP()
    ...
    Notify("RAGDOLL TP Enabled (with delays: "..DuelTP.TP_DELAY_1.."s + "..DuelTP.TP_DELAY_2.."s)")
end
...
initWPParts()
ApplyLoadedState()
Notify("H2N V9.9 | No Delay Auto Duel | TP Delays: "..DuelTP.TP_DELAY_1.."s + "..DuelTP.TP_DELAY_2.."s")
print("=" .. string.rep("=", 60))
print("📌 AUTO DUEL PATHS (No Delay):")
print("✓ LEFT: L1 → L2 → L1 → R1 → R2")
print("✓ RIGHT: R1 → R2 → R1 → L1 → L2")
print("=" .. string.rep("=", 60))
print("📌 RAGDOLL TP DELAYS:")
print("✓ TP to first point → wait "..DuelTP.TP_DELAY_1.."s → TP to second point → wait "..DuelTP.TP_DELAY_2.."s → Auto Play starts")
print("=" .. string.rep("=", 60))
print("📌 CONTROLS:")
