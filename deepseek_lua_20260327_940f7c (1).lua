...
local saveConfigBtn
local swapDirectionText = nil  -- للنص المجاور لزر Swap
...
-- ==================== ANTI RAGDOLL (الأصلي من K7 - شغال كويس) ====================
...
-- ==================== UNWALK (معدل - فقط إزالة الأنيميشن، آمن ولا يسبب موت) ====================
...
local function stopUnwalk()
    ...
    if c and savedAnimations.Animate then
        local newAnim = savedAnimations.Animate:Clone()
        newAnim.Parent = c
        savedAnimations.Animate = nil
    end
end

-- ==================== FLOAT SYSTEM (الأصلي من K7 - شغال كويس) ====================
...
local function startAutoLeft()
    ...
    autoLeftConnection = RunService.Heartbeat:Connect(function()
        ...
        -- إذا كنا في وضع الانتظار
        if waitingForStop then
            local waitTime = (autoLeftPhase == 2 and 0.2 or 0.1)
            if tick() - stopStartTime >= waitTime then
        ...
        if autoLeftPhase == 1 then
            ...
            if (tgt - rp.Position).Magnitude < 1.5 then
                waitingForStop = true
                stopStartTime = tick()
        ...
        elseif autoLeftPhase == 3 then
            ...
            if MobileButtons and MobileButtons["A.LEFT"] then MobileButtons["A.LEFT"].setState(false) end
            faceSouth()
        end
    end)
end
...
local function startAutoRight()
    ...
    autoRightConnection = RunService.Heartbeat:Connect(function()
        ...
        if waitingForStop then
            local waitTime = (autoRightPhase == 2 and 0.2 or 0.1)
            if tick() - stopStartTime >= waitTime then
                waitingForStop = false
        ...
        elseif autoRightPhase == 3 then
            ...
            if MobileButtons and MobileButtons["A.RIGHT"] then MobileButtons["A.RIGHT"].setState(false) end
            faceNorth()
        end
    end)
end
...
-- ==================== SWAP DIRECTION ROW ====================
local swapRow = Instance.new("Frame")
swapRow.Size = UDim2.new(1, 0, 0, deviceType == "Mobile" and 54 or 44)
swapRow.BackgroundColor3 = C_ROW_BG
swapRow.BorderSizePixel = 0
swapRow.LayoutOrder = O()
swapRow.Parent = vstack

Instance.new("UICorner", swapRow).CornerRadius = UDim.new(0, 10)

local swapLabel = Instance.new("TextLabel", swapRow)
swapLabel.Size = UDim2.new(0.6, 0, 1, 0)
swapLabel.Position = UDim2.new(0, 14, 0, 0)
swapLabel.BackgroundTransparency = 1
swapLabel.Text = "Swap Direction"
swapLabel.TextColor3 = C_PURPLE
swapLabel.Font = Enum.Font.GothamBold
swapLabel.TextSize = 13
swapLabel.TextXAlignment = Enum.TextXAlignment.Left
swapLabel.ZIndex = 5

-- النص المجاور لزر Swap (يتغير عند الضغط)
swapDirectionText = Instance.new("TextLabel", swapRow)
swapDirectionText.Size = UDim2.new(0, 60, 0, 26)
swapDirectionText.Position = UDim2.new(1, -70, 0.5, -13)
swapDirectionText.BackgroundColor3 = Color3.fromRGB(14, 0, 22)
swapDirectionText.Text = "LEFT"
swapDirectionText.TextColor3 = C_PURPLE2
swapDirectionText.Font = Enum.Font.GothamBold
swapDirectionText.TextSize = 12
swapDirectionText.ZIndex = 5
Instance.new("UICorner", swapDirectionText).CornerRadius = UDim.new(0, 8)
...
local function updateSwapText()
    swapDirectionText.Text = currentDirection
end
...
swapBtn.MouseButton1Click:Connect(function()
    ...
    updateSwapText()
    Notify("Swap Direction: " .. currentDirection)
end)
...
swapBtn.MouseEnter:Connect(function()
    playTick()
    TweenService:Create(swapLabel, TweenInfo.new(0.1), {TextColor3 = C_PURPLE2}):Play()
    TweenService:Create(swapRow, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(18, 0, 26)}):Play()
end)
swapBtn.MouseLeave:Connect(function()
    TweenService:Create(swapLabel, TweenInfo.new(0.1), {TextColor3 = C_PURPLE}):Play()
    TweenService:Create(swapRow, TweenInfo.new(0.12), {BackgroundColor3 = C_ROW_BG}):Play()
end)

local swapGap = Instance.new("Frame")
swapGap.Size = UDim2.new(1, 0, 0, 5)
swapGap.BackgroundTransparency = 1
swapGap.BorderSizePixel = 0
swapGap.Parent = vstack
...
print("✓ Auto Left/Right: Stop 0.1s at L1/R1, 0.2s at L2/R2")
print("✓ Unwalk: Only removes animations, no death or speed issues")
print("✓ Swap button: Changes direction text next to it")
print("✓ Anti-Ragdoll and Float: Original K7 version (working well)")