-- H2N V9.9 | Ultimate Edition | Auto Duel Fixed | No BodyGyro | Fixed Toggles
...
-- ====== AUTO DUEL (تم الإصلاح - حل مشكلة التعلق عند L2) ======
...
-- مسافة الكشف: 0.6 مع إضافة قوة دفع بسيطة
...
local function updateAutoPlayLeft()
    ...
    if reached then
        ...
        Notify("✓ Reached " .. target .. " (" .. aplPhase .. "/5)")
    end
end
...
local function updateAutoPlayRight()
    ...
    if reached then
        ...
        h.AssemblyLinearVelocity = Vector3.new(0, h.AssemblyLinearVelocity.Y, 0)
        if Hum then Hum:Move(Vector3.zero, false) end
        Notify("✓ Reached " .. target .. " (" .. aprPhase .. "/5)")
    end
end
...
-- ====== AUTO TRACK (بدون BodyGyro - لتجنب البان) ======
local trackConn = nil
...
-- Menu Frame (قابل للسحب فقط من العنوان)
...
-- ====== MAKE TOGGLE (نفس طريقة عمل زر Float - تعمل بشكل صحيح) ======
...
initWPParts()
ApplyLoadedState()
Notify("H2N V9.9 | No BodyGyro | Drag Only Title | Fixed Toggles")
print("=" .. string.rep("=", 60))
print("📌 AUTO DUEL PATHS (With Delay Fix):")
...
print("✓ Drag Menu: Only from title bar")
