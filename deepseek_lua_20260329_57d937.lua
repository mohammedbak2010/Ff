...
-- المسارات (تم إصلاحها لمنع القلتش)
local LEFT_ROUTE = {"L1", "L2", "R1", "R2"}
local RIGHT_ROUTE = {"R1", "R2", "L1", "L2"}
...
local function updateAutoPlayLeft()
    if not State.AutoPlayLeft or isTeleporting then 
        if aplConn then 
            aplConn:Disconnect()
            aplConn = nil
        end
...
local function updateAutoPlayRight()
    if not State.AutoPlayRight or isTeleporting then 
        if aprConn then 
            aprConn:Disconnect()
            aprConn = nil
        end
...
local function StartAutoPlayLeft()
    ...
    Notify("▶ Auto Play Left Started: L1 → L2 → R1 → R2")
end
...
local function StartAutoPlayRight()
    ...
    Notify("▶ Auto Play Right Started: R1 → R2 → L1 → L2")
end

-- ====== RAGDOLL TP (تحسين التيليبورت) ======
local function setPhaseFromPoint(side, startPoint)
    if side == "LEFT" then
        if startPoint == "L2" then aplPhase = 2
        elseif startPoint == "L1" then aplPhase = 1
        elseif startPoint == "R1" then aplPhase = 3
        elseif startPoint == "R2" then aplPhase = 4
        else aplPhase = 1 end
    else
        if startPoint == "R2" then aprPhase = 2
        elseif startPoint == "R1" then aprPhase = 1
        elseif startPoint == "L1" then aprPhase = 3
        elseif startPoint == "L2" then aprPhase = 4
        else aprPhase = 1 end
    end
end
...
local function teleportSequence(side)
    ...
    -- تعطيل Auto Duel مؤقتاً أثناء التيليبورت
    local wasLeft = State.AutoPlayLeft
    local wasRight = State.AutoPlayRight
    
    if wasLeft then StopAutoPlayLeft() end
    if wasRight then StopAutoPlayRight() end
    
    -- Teleport to first point (L1 or R1) مع تأخير 1.5 ثانية
    hrp.CFrame = CFrame.new(points[1])
    task.wait(1.5)
    
    -- Teleport to second point (L2 or R2) مع تأخير 1 ثانية
    ...
    -- إعادة تشغيل Auto Dalk بعد التيليبورت
    if wasLeft then
        State.AutoPlayLeft = true
        aplPhase = 1
...
local function onRagdollHit()
    ...
    if success and finalPoint then
        ...
        Notify("⚡ TP → " .. pointName .. " | Delay L1:1.5s L2:1s")
        return true
    end
    return false
end
...
local function startDuelTP()
    ...
    Notify("RAGDOLL TP Enabled | Delays: L1=1.5s, L2=1s")
end
...
initWPParts()
ApplyLoadedState()
Notify("H2N V9.9 | TP Delays: L1=1.5s, L2=1s | Fixed 4-point path")
print("=" .. string.rep("=", 60))
print("📌 AUTO DUEL PATHS (FIXED - NO GLITCH):")
print("✓ LEFT: L1 → L2 → R1 → R2 (4 نقاط فقط)")
print("✓ RIGHT: R1 → R2 → L1 → L2 (4 نقاط فقط)")
print("=" .. string.rep("=", 60))
print("📌 RAGDOLL TP DELAYS:")
print("✓ Teleport to L1/R1 → 1.5 ثانية انتظار")
print("✓ Teleport to L2/R2 → 1.0 ثانية انتظار")
print("✓ Auto Duel يبدأ فوراً بعد انتهاء التأخير")
print("=" .. string.rep("=", 60))
print("📌 CONTROLS:")
