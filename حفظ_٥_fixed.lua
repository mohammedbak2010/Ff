...
local function updatePostTPDuel()
    ...
    -- فاز 1: نقطة البداية (L2/R2) -> سرعة عالية (اقتراب)
    -- فاز 2-4: باقي النقاط -> سرعة الرجوع
    local spd = (postTPPhase == 1) and DUEL_APPROACH_SPD or DUEL_RETURN_SPD
    local reached = MoveToPoint(h, targetPos, spd)
...
local function StartPostTPDuel(side)
    ...
    if postTPActive then
        StopPostTPDuel()
    end

    -- تحديد المسار بناءً على الجهة (4 نقاط)
    if side == "LEFT" then
        postTPRoute = POST_TP_LEFT_ROUTE
        Notify("⚡ Post-TP: L2→L1→R1→R2")
    else
        postTPRoute = POST_TP_RIGHT_ROUTE
        Notify("⚡ Post-TP: R2→R1→L1→L2")
    end
...
local function onRagdollHit()
    ...
    -- إيقاف الأوتو دولز العادي فوراً
    if State.AutoPlayLeft then StopAutoPlayLeft() end
    if State.AutoPlayRight then StopAutoPlayRight() end
    -- إيقاف Post-TP Duel القديم إذا كان شغال
    ...
    if success then
        -- انتظار خروج الشخصية من حالة الـ ragdoll قبل بدء الأوتو دولز
        local char = LP.Character
        if char then
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum then
                local timeout = tick() + 2 -- انتظار أقصاه 2 ثانية
                while tick() < timeout do
                    local st = hum:GetState()
                    if st ~= Enum.HumanoidStateType.Ragdoll and
                       st ~= Enum.HumanoidStateType.FallingDown and
                       st ~= Enum.HumanoidStateType.Physics then
                        break
                    end
                    task.wait(0.05)
                end
                -- انتظار إضافي صغير للتأكد من استقرار الشخصية
                task.wait(0.08)
            end
        end
...
local function startDuelTP()
    ...
    duelTPConn = RunService.Heartbeat:Connect(function()
        ...
        -- نشغّل onRagdollHit فقط عند الانتقال من false إلى true (لحظة الضربة)
