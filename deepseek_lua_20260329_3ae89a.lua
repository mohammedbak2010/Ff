-- H2N V9.9 | Ultimate Edition | No Delay | Anti-Kick | Smooth Movement
...
local function LoadEverything()
    ...
    if data.DuelTP then 
        ...
        if DuelTP.MyHome then DuelTP.HomeSet = true end
    end
...
-- ====== FLOAT (مع حماية من الطرد) ======
local floatUpdateCounter = 0
local function startFloat()
    ...
    FloatConn = RunService.Heartbeat:Connect(function()
        ...
        -- حماية من الطرد: تحديثات أقل تكراراً
        floatUpdateCounter = floatUpdateCounter + 1
        if floatUpdateCounter % 2 == 0 then return end
        ...
        if diff > 0.3 then
            local upSpeed = math.min(diff * 10, 20)
...
-- ====== AUTO DUEL (بدون انتظار، يصل لنص النقطة) ======
local aplConn, aprConn = nil, nil
local aplPhase, aprPhase = 1, 1
local lastAplUpdate, lastAprUpdate = 0, 0
...
-- ====== AUTO TRACK (مع حماية من الطرد) ======
local trackConn = nil
local trackBodyGyro = nil
local trackUpdateCounter = 0
...
local function StartAutoTrack()
    ...
    trackConn = RunService.Heartbeat:Connect(function()
        ...
        -- حماية من الطرد: تحديثات أقل تكراراً
        trackUpdateCounter = trackUpdateCounter + 1
        if trackUpdateCounter % 3 == 0 then return end
...
initWPParts()
ApplyLoadedState()
Notify("H2N V9.9 | Anti-Kick | No Delay Auto Duel")
print("=" .. string.rep("=", 60))
...
print("=" .. string.rep("=", 60))
print("📌 IMPORTANT:")
print("✓ Auto Duel moves when within 3.5 studs of point (center detection)")
print("✓ No waiting between points - instant transition")
print("✓ Anti-kick protection added (reduced update rates)")
print("✓ All menu toggles now work correctly")
print("=" .. string.rep("=", 60))