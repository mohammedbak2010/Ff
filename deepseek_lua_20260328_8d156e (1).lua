-- H2N V9.8 | Fixed | Ragdoll TP with stops | Auto Dues unchanged | Full Save
...
-- ====== RAGDOLL TP (مع توقفات 0.1 عند L1/R1 و 0.2 عند L2/R2) ======
...
local function teleportSequence(side)
    ...
    -- النقل للنقطة الأولى (L1/R1) - وقوف 0.1 ثانية
    hrp.CFrame = CFrame.new(points[1])
    task.wait(0.1)

    -- النقل للنقطة الثانية (L2/R2) - وقوف 0.2 ثانية
...
do
    ...
    if sp then
        ...
        container.Position = UDim2.new(1, -(bSize + 10), 0.22 + 3 * 0.19, 0) -- أسفل زر RAGDOLL TP
    end
...
print("H2N V9.8 - Fixed | Auto Dues Original | Ragdoll TP & FLOAT as separate side buttons | All side buttons draggable")