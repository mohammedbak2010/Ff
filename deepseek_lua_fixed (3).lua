-- H2N V9.9 | Ultimate Edition | Auto Duel No Delay | No Home Set | Fixed Toggles
...
local function teleportSequence(side)
    ...
    task.spawn(function()
        -- وقف أي أوتو دولز شتغل قبل الضربة
        if State.AutoPlayLeft then StopAutoPlayLeft() end
        if State.AutoPlayRight then StopAutoPlayRight() end

        -- خطوة 1
        local h1 = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
        if h1 then h1.CFrame = CFrame.new(points[1]) end
        Notify("⚡ TP → " .. (side == "LEFT" and "L1" or "R1"))
        task.wait(0.2)

        -- خطوة 2
        local h2 = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
        if h2 then h2.CFrame = CFrame.new(points[2]) end
        Notify("⚡ TP → " .. (side == "LEFT" and "L2" or "R2"))
        task.wait(0.15)

        -- تشغيل أوتو دولز بـ 4 نقاط (TP mode) من البداية
        if not State.DuelTP then duelTPInProgress = false; return end

        if side == "LEFT" then
            State.AutoPlayLeft = true
            aplTPMode = true   -- 4 نقاط: L2→L1→R1→R2
            aplPhase = 1
            if Hum then Hum.AutoRotate = false end
            if aplConn then aplConn:Disconnect(); aplConn = nil end
            aplConn = RunService.Heartbeat:Connect(updateAutoPlayLeft)
            ...
            State.AutoPlayRight = true
            aprTPMode = true   -- 4 نقاط: R2→R1→L1→L2
            aprPhase = 1
            if Hum then Hum.AutoRotate = false end
            if aprConn then aprConn:Disconnect(); aprConn = nil end
            aprConn = RunService.Heartbeat:Connect(updateAutoPlayRight)
...
initWPParts()
ApplyLoadedState()
Notify("H2N V9.9 | No Delay Auto Duel | No Home Set | Fixed Toggles")
print("=" .. string.rep("=", 60))
