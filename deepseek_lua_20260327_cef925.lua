-- H2N V9.8 | Delta Mobile | Anti-Ragdoll + Auto Taunt + Ragdoll TP
...
local State = {
    ...
    SpinBody=false, DuelTP=false, FloatEnabled=false, Unwalk=false,
}
...
local function SaveConfig()
    local cfg = {
        ...
        Keys = {
            ...
            DuelTP=Keys.DuelTP.Name, Unwalk=Keys.Unwalk.Name
        }
    }
...
local function teleportSequence(side)
    ...
    -- النقل إلى النقطة الأولى (مدة 0.08)
    hrp.CFrame = CFrame.new(points[1])
    task.wait(0.08)
    -- النقل إلى النقطة الثانية (مدة 0.1)
...
MakeToggle(protect, "Anti Ragdoll", pi, function(s) if s then StartAntiRagdoll() else StopAntiRagdoll() end end, function() return State.AntiRagdoll end); pi = pi + 1
...
MakeToggle(protect, "UNWALK", pi, function(s) if s then StartUnwalk() else StopUnwalk() end end, function() return State.Unwalk end); pi = pi + 1
...
local sideNames = {"AUTO PLAY LEFT", "AUTO PLAY RIGHT", "AUTO TRACK", "RAGDOLL TP", "FLOAT", "UNWALK"}
...
MakeKeybind(sTab, "Unwalk Key", "Unwalk", si); si = si + 1
...
-- الأزرار الجانبية (بدون Speed Boost)
            ...
            if flatMag > 350 then
                ...
                if not State.Unwalk then
                    ...
                    if md.Magnitude > 0.05 and flatV < targetSpeed - 5 then
                        ...
                        HRP.AssemblyLinearVelocity = Vector3.new(tX, v.Y, tZ)
                    end
                else
                    Hum.WalkSpeed = 0
...
initWPParts()
Notify("H2N V9.8 | Delta Mobile | Anti-Ragdoll + Unwalk + Ragdoll TP")