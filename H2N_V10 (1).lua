...
local function teleportSequence(side)
    ...
    -- التيليبورت للنقطة الأولى (L1 أو R1) مع تأخير 0.2
    hrp.CFrame = CFrame.new(points[1])
    task.wait(0.2)

    -- التيليبورت للنقطة الثانية (L2 أو R2) مع تأخير 0.15
...
local function StartPostTPDuel(side)
    ...
    if side == "LEFT" then
        -- وصلنا L2، نكمل: L2 → L1 → R1 → R2
        postTPRoute = POST_TP_LEFT_ROUTE
        Notify("⚡ Post-TP Auto: L2 → L1 → R1 → R2")
    else
        -- وصلنا R2، نكمل: R2 → R1 → L1 → L2
        postTPRoute = POST_TP_RIGHT_ROUTE
        Notify("⚡ Post-TP Auto: R2 → R1 → L1 → L2")
    end
...
local function onRagdollHit()
    ...
    if success then
        -- الأوتو دولز العادي (left/right) لا نوقفه ولا نلمسه
        -- فقط نشغّل Post-TP Duel المستقل
...
local function stopDuelTP()
    State.DuelTP = false
    if duelTPConn then duelTPConn:Disconnect(); duelTPConn = nil end
    -- إيقاف Post-TP Duel لو كان شغال
...
initWPParts()
ApplyLoadedState()
Notify("H2N V10 | Dual AutoDuel | TP: 0.2s + 0.15s | Post-TP: 4pts independent")
print("=" .. string.rep("=", 60))
print("📌 AUTO DUEL PATHS - NORMAL (5 نقاط):")
print("✓ LEFT:  L1 → L2 → L1 → R1 → R2")
print("✓ RIGHT: R1 → R2 → R1 → L1 → L2")
print("=" .. string.rep("=", 60))
print("📌 POST-TP AUTO DUEL (4 نقاط) - مستقل تماماً:")
print("✓ بعد TP LEFT:  L2 → L1 → R1 → R2")
print("✓ بعد TP RIGHT: R2 → R1 → L1 → L2")
print("=" .. string.rep("=", 60))
print("📌 RAGDOLL TP DELAYS:")
print("✓ نقطة 1 (L1/R1) → تأخير 0.2 ثانية")
print("✓ نقطة 2 (L2/R2) → تأخير 0.15 ثانية")
print("=" .. string.rep("=", 60))
print("📌 CONTROLS:")
print("✓ G=AutoLeft | H=AutoRight | T=Track | J=Jump | R=RagdollTP | K=AntiRagdoll")
print("=" .. string.rep("=", 60))
