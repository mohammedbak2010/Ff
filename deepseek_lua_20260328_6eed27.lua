...
-- ====== RAGDOLL TP SIDE BUTTON (WITH SWAP) ======
do
    ...
    -- Drag system
    local pressing, hasMoved, dragStart, btnStart, activeInput = false, false, nil, nil, nil
    local activeTouchForDragR = nil

    container.InputBegan:Connect(function(input)
        local t = input.UserInputType
        if t ~= Enum.UserInputType.Touch and t ~= Enum.UserInputType.MouseButton1 then return end
        if activeTouchForDragR ~= nil then return end
        if pressing then return end
        pressing = true
        hasMoved = false
        activeInput = input
        activeTouchForDragR = input
        dragStart = input.Position
    ...
    UIS.InputChanged:Connect(function(input)
        ...
        if delta.Magnitude > 6 then
            hasMoved = true
            container.Position = UDim2.new(btnStart.X.Scale, btnStart.X.Offset + delta.X, btnStart.Y.Scale, btnStart.Y.Offset + delta.Y)
        end
    end)
    ...
    local function updateRagdollButtonUI()
        ...
        dirLbl.Text = (DuelTP.EnemySide == "RIGHT") and "→ R" or "← L"
        if arrowBtn then arrowBtn.Text = (DuelTP.EnemySide == "RIGHT") and "R" or "L" end
    end
    ...
    -- Main click button
    local mainClick = Instance.new("TextButton", mainF)
    mainClick.Size = UDim2.new(1, 0, 1, 0)
    mainClick.BackgroundTransparency = 1
    mainClick.Text = ""

    mainClick.MouseButton1Click:Connect(toggleRagdollTP)
    mainClick.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.Touch then toggleRagdollTP() end
    end)
