...
local function teleportSequence(side)
    ...
    -- Teleport to first point (L1 or R1) مع تأخير 0.15 ثانية
    hrp.CFrame = CFrame.new(points[1])
    task.wait(0.15)
    
    -- Teleport to second point (L2 or R2) مع تأخير 0.1 ثانية
    ...
    -- إعادة تشغيل Auto Duel بعد التيليبورت فوراً
    if wasLeft then
        State.AutoPlayLeft = true
        aplPhase = 1
...
local function onRagdollHit()
    ...
    if success and finalPoint then
        ...
        Notify("⚡ TP → " .. pointName .. " | Delays: 0.15s + 0.1s")
        return true
    end
    return false
end
...
local function startDuelTP()
    ...
    Notify("RAGDOLL TP Enabled | Delays: 0.15s + 0.1s")
end
...
initWPParts()
ApplyLoadedState()
Notify("H2N V9.9 | TP Delays: 0.15s + 0.1s | 4-point path | Auto Duel starts instantly")
print("=" .. string.rep("=", 60))
print("📌 AUTO DUEL PATHS (4 نقاط فقط - بدون قلتش):")
print("✓ LEFT: L1 → L2 → R1 → R2")
print("✓ RIGHT: R1 → R2 → L1 → L2")
print("=" .. string.rep("=", 60))
print("📌 RAGDOLL TP DELAYS:")
print("✓ Teleport to L1/R1 → 0.15 ثانية انتظار")
print("✓ Teleport to L2/R2 → 0.1 ثانية انتظار")
